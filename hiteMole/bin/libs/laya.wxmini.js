...
(function(window,document,Laya){
	...
	var SoundManager=laya.media.SoundManager,Stage=laya.display.Stage,URL=laya.net.URL,Utils=laya.utils.Utils;
	...
	*/
	__proto.load=function(url,type,cache,group,ignoreCache){
		...
		if ((MiniLoader._fileTypeArr.indexOf(urlType)!=-1)){
			...
			if (!MiniFileMgr.getFileInfo(url)){
				var fileNativeUrl=url.split(URL.basePath)[0];
				if(URL.basePath=="")
					fileNativeUrl=url;
				if(url.indexOf("http://")!=-1 || url.indexOf("https://")!=-1){
	...
	MiniLoader.onReadNativeCallBack=function(encoding,url,type,cache,group,ignoreCache,thisLoader,errorCode,data){
		...
		if (!errorCode){
			if (type==/*laya.net.Loader.JSON*/"json" || type==/*laya.net.Loader.ATLAS*/"atlas"){
				thisLoader.onLoaded(MiniAdpter.getJson(data.data));
				}else if (type==/*laya.net.Loader.XML*/"xml"){
				thisLoader.onLoaded(Utils.parseXMLFromString(data.data));
				}else {
				thisLoader.onLoaded(data.data);
			}
			}else if (errorCode==1){
			MiniAdpter.EnvConfig.load.call(thisLoader,url,type,cache,group,ignoreCache);
		}
...
var MiniFileMgr=(function(_super){
	...
	MiniFileMgr.getFileInfo=function(fileUrl){
		...
		var fileObj=MiniFileMgr._filesKey[fileNativePath];
		if (fileObj==null)
			return null;
		else
		return fileObj;
	...
	MiniFileMgr.downImg=function(fileUrl,callBack,readyUrl){
		...
		var downloadTask=MiniFileMgr.wxdown({url:fileUrl,success:function (data){
				if (data.statusCode===200){
					MiniFileMgr.rename(data.tempFilePath,readyUrl,callBack);
				}
				},fail:function (data){
				callBack !=null && callBack.runWith([1,data]);
		}});
	...
	MiniFileMgr.rename=function(tempFilePath,readyUrl,callBack){
		var temp=tempFilePath.split("/");
		var tempFileName=temp[temp.length-1];
		var fileurlkey=readyUrl.split("?")[0];
		var fileObj=MiniFileMgr.getFileInfo(readyUrl);
		var saveFilePath=MiniFileMgr.getFileNativePath(tempFileName);
		MiniFileMgr.fs.rename({oldPath:tempFilePath,newPath:saveFilePath,success:function (data){
				if (!fileObj){
					MiniFileMgr.onSaveFile(readyUrl,tempFileName);
					callBack !=null && callBack.runWith([0]);
					}else {
	...
	MiniFileMgr.onSaveFile=function(readyUrl,md5Name){
		var fileurlkey=readyUrl.split("?")[0];
		MiniFileMgr._filesKey[fileurlkey]={md5:md5Name,readyUrl:readyUrl};
	}
